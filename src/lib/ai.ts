...
export async function generateDishCaption(name: string, calories: number): Promise<string> {
...
export async function suggestDishInfo(name: string): Promise<{ calories: number; keywords: string }> {
  const { endpoint, model, key } = getAIEnv()
  if (!name) throw new Error('缺少菜名')
  const res = await fetchWithTimeout(endpoint, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${key}` },
    body: JSON.stringify({
...
export function keywordsToLoremFlickrUrl(keywords: string, w = 400, h = 300) {
...
export function dishNameToTags(name: string): string {
  const n = String(name)
  ...
  for (const [re, tag] of pairs) if (re.test(n)) return tag
  return 'food,dish'
}

export function imageUrlForDishName(name: string, w = 400, h = 300) {
  const tags = dishNameToTags(name)
  return `https://loremflickr.com/${w}/${h}/${tags}?random=${Math.random()}`
}

export async function chatRecommend(messages: { role: 'user' | 'assistant' | 'system'; content: string }[]): Promise<string> {
  const { endpoint, model, key } = getAIEnv()
  const res = await fetchWithTimeout(endpoint, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${key}` },
...
export async function generateShoppingList(context: string): Promise<Array<{ item: string; qty: string }>> {
  const { endpoint, model, key } = getAIEnv()
  const res = await fetchWithTimeout(endpoint, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${key}` },
  ...
  return arr.map((x: any) => ({ item: String(x.item || x.name || '食材'), qty: String(x.qty || x.quantity || '适量') })).slice(0, 20)
}
function getAIEnv() {
  ...
  const keysRawEnv = (import.meta.env.VITE_AI_KEYS as string | undefined) || ''
  const singleKeyEnv = (import.meta.env.VITE_AI_KEY as string | undefined) || ''
  const key = [keysRawEnv, singleKeyEnv]
    ...
    .map((s) => s.trim())
    .filter(Boolean)[0]
  if (!endpoint || !model || !key) throw new Error('AI 环境变量未配置完整')
